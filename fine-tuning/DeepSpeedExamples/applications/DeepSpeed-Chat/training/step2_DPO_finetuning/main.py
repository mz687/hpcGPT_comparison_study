...
from dschat.utils.utils import print_rank_0, to_device, save_hf_format, set_random_seed, get_all_reduce_mean, get_optimizer_grouped_parameters, save_zero_three_model, load_hf_tokenizer, report_memory
...
def main():
    ...
    print_rank_0(f"tokenizer.eos_token: {tokenizer.eos_token}", args.global_rank)
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        # save checkpoint
        if epoch % args.save_interval == 0 and args.output_dir is not None:
            os.makedirs(args.output_dir, exist_ok=True)
            if args.global_rank == 0:
                print_rank_0(f"Saving checkpoint at epoch {epoch}", args.global_rank)
            model_linear = convert_lora_to_linear_layer(model)

            if args.global_rank == 0:
                save_hf_format(model_linear, tokenizer, args)

            if args.zero_stage == 3:
                # For zero stage 3, each gpu only has a part of the model, so we need a special save function
                save_zero_three_model(model_linear,
                                      args.global_rank,
                                      args.output_dir,
            ...
            model_linear.save_checkpoint(args.output_dir, ckpt_id, client_state=client_sd)
            if args.global_rank == 0:
