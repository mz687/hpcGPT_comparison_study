...
def report_memory(name, rank=None):
...
def get_tokenizer(model_name_or_path, fast_tokenizer=True):
    if "Llama-2" in model_name_or_path: ## or"llama"
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_name_or_path, fast_tokenizer=fast_tokenizer)
        # Use specially reserved tokens for fine-tuning (https://github.com/meta-llama/llama3/issues/77)
        # if "Llama-3.1" in model_name_or_path:
        #     tokenizer.add_special_tokens({'pad_token': '<|reserved_special_token_0|>'})
        #     print("Setting tokenizer.pad_token as {}".format(tokenizer.pad_token))
        # else:
...
def load_hf_tokenizer(model_name_or_path,
    ...
    if os.path.exists(model_name_or_path):
        ...
        if os.path.exists(model_json):
            ...
            raise Exception(f"Cannot find config.json in path: {model_name_or_path}")
    else:
        # 
        # model_name_or_path = 'meta-llama/Llama-3.1-8B-Instruct' if 'meta-llama/Llama-3.1-8B' == model_name_or_path else model_name_or_path
        tokenizer = get_tokenizer(model_name_or_path,
                                  fast_tokenizer=fast_tokenizer)
        if model_name_or_path == 'meta-llama/Llama-3.1-8B':
            chat_template = open('/work/09308/zhengmk/finetune_llama3.1_DL_assignment/DeepSpeedExamples/applications/DeepSpeed-Chat/dschat/utils/data/llama-3.1-chat-template.jinja', 'r').read()
...
def save_hf_format(model, tokenizer, args, sub_folder=""):
    ...
    try:
        ...
        try:
            ...
            # If save_pretrained doesn't work either, try to save the entire tokenizer.
            import pickle
            with open(os.path.join(output_dir, "tokenizer.pkl"), "wb") as f:
                pickle.dump(tokenizer, f)
            print("Warning: Saved tokenizer as a pickle file.")
