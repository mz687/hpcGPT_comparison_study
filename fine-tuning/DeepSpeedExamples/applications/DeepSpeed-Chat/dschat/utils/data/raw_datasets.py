...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name, tokenizer):
    ...
    # convert 'Human:...\nAssistant:...' to proper tokenizer template
    def process_prompt(self, sample):
...
class DahoasRmstaticDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name, tokenizer):
        super().__init__(output_path, seed, local_rank, dataset_name, tokenizer)
    ...
    def get_prompt(self, sample):
        processed_sample = self.process_prompt(sample['prompt'])
        processed_sample = [{'role':'system', 'content':'You are an assistant for answering general-knowledge questions.'}, 
                            *processed_sample]
        return self.tokenizer.apply_chat_template(processed_sample,
                                                  tokenize=False,
...
class YitingxieRlhfrewarddatasetsDataset(PromptRawDataset):
    ...
    def get_prompt(self, sample):
        ...
        if sample['prompt'] is not None:
            processed_sample = self.process_prompt(sample['prompt'])
            processed_sample = [{'role':'system', 'content':'You are an HPC assistant for questions submitted by HPC users.'}, 
                                *processed_sample]
            return self.tokenizer.apply_chat_template(processed_sample,
                                                    tokenize=False,
                                                    add_generation_prompt=True)
...
class LocalJsonFileDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name, chat_path, tokenizer):
        super().__init__(output_path, seed, local_rank, dataset_name, tokenizer)
