...
class LocalLLM(DeepEvalBaseLLM):
    ...
    async def a_generate(self, prompt: str, schema=None, max_length=None):
        """Asynchronous inference with proper schema handling"""
        ...
        try:
            ...
            if schema and hasattr(schema, '__annotations__'):
                ...
                for key in schema.__annotations__:
                    if key not in json_data:
                        if key == 'statements' or key == 'verdicts':
                            json_data[key] = []
                        elif key == 'reason':
            ...
            # Create and return proper schema instance if need
            if schema and hasattr(schema, '__annotations__'):
                empty_data = {
...
def evaluate_relevance(test_cases, eval_model, threshold=0.5):
    ...
    for i, case in enumerate(tqdm(test_cases, desc="Evaluating test cases")):
        ...
        # Evaluate
        try:
            evaluation_result = evaluate([test_case], [relevance_metric])
            print("Type of evaluation_result:", type(evaluation_result))  #
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate relevance of responses")
    ...
    results = evaluate_relevance(test_cases, eval_llm, args.threshold)

    # Calculate average score
    avg_score = sum(r["relevance_score"] for r in results) / len(results) if results else 0
    print(f"Average relevance score for {file_name}: {avg_score:.4f}")
 
    # Save results
    output_path = os.path.join(args.output_dir, f"relevance_{file_name}")
