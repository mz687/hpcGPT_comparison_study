...
def build_vector_store(docs_list, device):
    ...
    # Create retriever (return top-1 most relevant doc chunks)
...
if __name__ == '__main__':
    ...
    # Build vector store
    ICL_file_path = args.ICL_file_path
    RAG_file_path = args.RAG_file_path
    # load ICL data and build ICL vector store
    ICL_docs_list = load_ICL_files(ICL_file_path)
    ICL_retriever = build_vector_store(ICL_docs_list,
                                       args.device)
    # load RAG data and build RAG vector store
    RAG_docs_list = load_files(RAG_file_path)
    RAG_retriever = build_vector_store(RAG_docs_list,
                                       args.device)
    ...
    rag_prompt = """
    Here is some context that might be useful for answering the question:
    ...
    for i, ticket in enumerate(questions):
        ...
        icl_docs = ICL_retriever.invoke(prompt)
        icl_docs_txt = format_docs(icl_docs)

        rag_docs = RAG_retriever.invoke(prompt)
        rag_docs_txt = format_docs(rag_docs)

        icl_prompt_template = icl_prompt.format(examples=icl_docs_txt)
        rag_prompt_template = rag_prompt.format(context=rag_docs_txt)
        ...
        if num_tokens > args.max_context_len:
            prompt_formatted['input_ids'] = prompt_formatted['input_ids'][:, 0:args.max_context_len-1].reshape(1,-1)
            prompt_formatted['attention_mask'] = prompt_formatted['attention_mask'][:, 0:args.max_context_len-1].reshape(1,-1)
        try:
            ...
            res['context'] = rag_docs_txt
            res['examples'] = icl_docs_txt
