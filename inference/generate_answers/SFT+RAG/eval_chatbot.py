...
if __name__ == '__main__':
    ...
    # Create Prompts
    system_prompt = "You are an HPC assistant for questions submitted by HPC users."
    ...
    for i, ticket in enumerate(questions):
        ...
        rag_prompt_template = rag_prompt.format(context=docs_txt)
        ...
        try:
            rag_prompt_formatted = tokenizer.apply_chat_template(template,
                                                                tokenize=True, 
                                                                return_dict=True, 
        ...
        num_tokens = rag_prompt_formatted["input_ids"].shape[1]
        ...
        if num_tokens > args.max_context_len:
            rag_prompt_formatted["input_ids"] = rag_prompt_formatted["input_ids"][:, 0:args.max_context_len-1].reshape(1,-1)
            rag_prompt_formatted["attention_mask"] = rag_prompt_formatted["attention_mask"][:, 0:args.max_context_len-1].reshape(1,-1)
        try:
            # generated = llm.invoke([HumanMessage(content=rag_prompt_formatted)])
            # generated = generated.split('Assistant:')[-1]

            generation = tokenizer.decode(model.generate(**rag_prompt_formatted,
                                            do_sample=False,
