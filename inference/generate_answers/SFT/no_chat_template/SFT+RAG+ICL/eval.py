...
def build_vector_store(docs_list, device):
    ...
    # Create retriever (return the most relevant doc chunks).
    # Due to constrained context_len = 2048
...
if __name__ == '__main__':
    ...
    Here is the context related to the question:

    {context} 

    Here is an example similar to the question:

    {example}

    Think carefully. 

    Use the example and context to provide an answer to this question. 
    ...
    for i, ticket in enumerate(questions):
        ...
        ICL_docs = ICL_retriever.invoke(question)
        ICL_docs_txt = format_docs(ICL_docs)

        RAG_docs = RAG_retriever.invoke(question)
        RAG_docs_txt = format_docs(RAG_docs)
        rag_prompt_formatted = rag_prompt.format(context=RAG_docs_txt, 
                                                 example=ICL_docs_txt,
        ...
        try:
            ...
            res['icl'] = ICL_docs_txt
            res['rag'] = RAG_docs_txt
